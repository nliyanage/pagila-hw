...
$$
SELECT category.name, count(category.name) as "count" FROM category JOIN film_category USING (category_id) JOIN film USING (film_id) JOIN language USING (language_id) WHERE language.name = $1 GROUP BY category.name ORDER BY name;
$$
LANGUAGE SQL
IMMUTABLE
